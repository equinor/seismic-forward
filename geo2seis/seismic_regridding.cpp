...
void SeismicRegridding::seismicRegridding(SeismicParameters &seismic_parameters) {
    ...
    if (seismic_parameters.model_settings->GetOutputReflections()) {
        seismic_parameters.seismicOutput()->writeReflections(seismic_parameters, false, false);
    }

    //-----------------Add white noise to reflections----------------------------
    if (seismic_parameters.model_settings->GetWhiteNoise()) {
        unsigned long seed = seismic_parameters.model_settings->GetSeed();
        double deviation = seismic_parameters.model_settings->GetStandardDeviation();
        addNoiseToReflections(seed, deviation, seismic_parameters.rGrids());
    }

    if (seismic_parameters.model_settings->GetOutputReflections() && seismic_parameters.model_settings->GetWhiteNoise()) {
        seismic_parameters.seismicOutput()->writeReflections(seismic_parameters, true, false);
    }
    ...
    bool find_for_ps = seismic_parameters.model_settings->GetPSSeismic();
    ...
    if (seismic_parameters.model_settings->GetNLayersFileName() != "") {
        NRLib::StormContGrid tmpgrid(seismic_parameters.model_settings->GetNLayersFileName());
        nt = tmpgrid.GetNK();
    }
    ...
    if (seismic_parameters.model_settings->GetOutputTimeSurfaces()) {
    ...
    //time grid:
    if (seismic_parameters.model_settings->GetOutputElasticParametersTimeSegy()) {
        seismic_parameters.seismicOutput()->writeElasticParametersTimeSegy(seismic_parameters);
    }

    if (seismic_parameters.model_settings->GetOutputExtraParametersTimeSegy()) {
        seismic_parameters.seismicOutput()->writeExtraParametersTimeSegy(seismic_parameters);
    }

    //depth grid:
    if (seismic_parameters.model_settings->GetOutputElasticParametersDepthSegy()) {
        seismic_parameters.seismicOutput()->writeElasticParametersDepthSegy(seismic_parameters);
    }

    if (seismic_parameters.model_settings->GetOutputExtraParametersDepthSegy()) {
        seismic_parameters.seismicOutput()->writeExtraParametersDepthSegy(seismic_parameters);
    }

    if (seismic_parameters.model_settings->GetOutputVp()) {
        seismic_parameters.seismicOutput()->writeVpVsRho(seismic_parameters, false);
    }

    if (seismic_parameters.model_settings->GetOutputZvalues()) {
        seismic_parameters.seismicOutput()->writeZValues(seismic_parameters, false);
    }

    if (seismic_parameters.model_settings->GetOutputTwt()) {
        seismic_parameters.seismicOutput()->writeTwt(seismic_parameters, false);
    }
...
void SeismicRegridding::findZValues(SeismicParameters &seismic_parameters) {
    ...
    bool use_corner_point = seismic_parameters.model_settings->GetUseCornerpointInterpol();
...
void SeismicRegridding::findVpAndR(SeismicParameters &seismic_parameters) {
    ...
    std::vector<double> constvp = seismic_parameters.model_settings->GetConstVp();
    std::vector<double> constvs = seismic_parameters.model_settings->GetConstVs();
    std::vector<double> constrho = seismic_parameters.model_settings->GetConstRho();

    std::vector<std::string> names = seismic_parameters.model_settings->GetParameterNames();
    std::vector<double> extra_parameter_default_values = seismic_parameters.model_settings->GetExtraParameterDefaultValues();
    std::vector<std::string> extra_parameter_names = seismic_parameters.model_settings->GetExtraParameterNames();
    NRLib::StormContGrid &zgrid = seismic_parameters.zGrid();
    //NRLib::RegularSurface<double> &toptime = seismic_parameters.topTime();

    double zlimit = seismic_parameters.model_settings->GetZeroThicknessLimit();
    bool ps_seis = seismic_parameters.model_settings->GetPSSeismic();
