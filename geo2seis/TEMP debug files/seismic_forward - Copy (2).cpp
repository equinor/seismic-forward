...
void SeismicForward::seismicForward(SeismicParameters &seismic_parameters) {
  ...
  if (seismic_parameters.modelSettings()->GetNMOCorr()){
    ...
    if (time_output || depth_output || timeshift_output) { //OK?
      ...
      std::vector<double> seis_sample_t(z_0.size());
      ...
      for (int il = 1350; il < 1351; il += il_step) {
        ...
        ++il_steps;
        xl_steps = 0;
        for (int xl = 2175; xl < 2176/*2225*/; xl += xl_step) {
        //for (int xl = xl_min; xl <= xl_max; xl +=xl_step) {
          ...
          if (nmo_depth_segy_ok || nmo_depth_stack_segy_ok || depth_storm_output) {
            ...
            //seismic_parameters.seismicOutput()->printVector(zgrid_vec_extrapol, "zgrid_vec_extrapol.txt");
            //seismic_parameters.seismicOutput()->printVector(twt_vec_extrapol, "twt_vec_extrapol.txt");
            if (nmo_depth_segy_ok) {
              ...
              for (size_t k = 0; k < z_0.size(); ++k) {
                seis_sample_v[xl_steps - 1][k] = nmo_depthgrid_pos[k][0];
              }
            }
          ...
          if (nmo_timeshift_segy_ok || nmo_timeshift_stack_segy_ok || timeshift_storm_output) {
            ...
            for (size_t k = 1; k < nzrefl; ++k) {
              ...
              if (twt_vec[k] == twt_vec[k-1]) {
                timeshiftgrid_vec[k] = timeshiftgrid_vec[k-1];
              }
            }
            if (nmo_timeshift_segy_ok) {
              convertSeis(twt_vec, twt_0, timeshiftgrid_vec, twt_0, nmo_timegrid_pos, nmo_timeshiftgrid_pos, seismic_parameters);
            ...
            if (nmo_timeshift_stack_segy_ok || timeshift_storm_output){
              convertSeis(twt_vec, twt_0, timeshiftgrid_vec, twt_0, nmo_timegrid_stack_pos, nmo_timeshiftgrid_stack_pos, seismic_parameters);
            }
      ...
      seismic_parameters.seismicOutput()->printMatrix(seis_sample_v, "seis_sample_NMO.txt");  
