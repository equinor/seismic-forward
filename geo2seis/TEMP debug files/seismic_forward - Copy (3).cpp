...
void SeismicForward::seismicForward(SeismicParameters &seismic_parameters) {
  ...
  if (seismic_parameters.modelSettings()->GetNMOCorr()){
    ...
    if (time_output || depth_output || timeshift_output) { //OK?
      ...
      std::vector<std::vector<double> > seis_sample_time(50);
      std::vector<std::vector<double> > seis_sample_timeshift(50);
      std::vector<std::vector<double> > seis_sample_depth(50);
      ...
      for (int il = 1350; il < 1351; il += il_step) {
        ...
        ++il_steps;
        xl_steps = 0;
        for (int xl = 2175; xl < 2225; xl += xl_step) {
                                     ...
                                     SeismicParameters &seismic_parameters) 
{
  ...
  //#endif
  std::vector<double> seis_sample(nz);
  std::vector<std::vector<double> > seis_sample_v(50);
  std::vector<double> seis_sample_t(nz);
  std::vector<double> seis_sample_z(nz);
  for (size_t i = 0; i < 50; ++i){
    seis_sample_v[i] = seis_sample_t;
  }
  std::ofstream fout;
  ...
  std::cout << "\n";
  fout.close();
  seismic_parameters.seismicOutput()->printMatrix(seis_sample_v, "seis_sample_OLD.txt"); 
  seismic_parameters.seismicOutput()->printVector(seis_sample_t, "seis_sample_t_OLD.txt");  
  seismic_parameters.seismicOutput()->printVector(seis_sample_z, "seis_sample_z_OLD.txt");
  // stop=omp_get_wtime();
