...
void SeismicForward::seismicForward(SeismicParameters &seismic_parameters) {
  ...
  if (seismic_parameters.modelSettings()->GetNMOCorr()){
    if (time_output || depth_output || timeshift_output) { //OK?
      ...
      std::vector<std::vector<double> > seis_sample_v(25);
      ...
      //for (int il = 1366; il < 1367; il += il_step) { //260, 550 = 1366,1833
        //for (int xl = 1832; xl < 1833; xl += xl_step) {
      for (int il = 1350; il < 1351; il += il_step) {
        for (int xl = 1750; xl < 1775; xl += xl_step) {
      //for (int il = il_min; il < il_max; il += il_step) {
        ...
        //for (int xl = xl_min; xl < xl_max; xl +=xl_step) {
          ...
          if (twtgrid(i, j, 0) != -999.0) {
            ...
            for (size_t k = 0; k < nzrefl; ++k) {
               refl_pos[k][0] = rgridvec[0](i,j,k);
            }
            seismic_parameters.seismicOutput()->printMatrix(refl_pos, "refl_pos_gridding.txt");
            ...
            if (i == 285 && j >= 882 && j < 907) {
              for (size_t k = 0; k < twt_0.size(); ++k) {
                seis_sample_v[j-882][k] = nmo_timegrid_pos[k][0];
              }
            }
      ...
      seismic_parameters.seismicOutput()->printMatrix(seis_sample_v, "seis_sample_v.txt");  
                                     ...
                                     SeismicParameters &seismic_parameters)
{
  ...
  seismic_parameters.seismicOutput()->printVector(seis_sample_t, "seis_sample_t_nmo.txt");  
                                     ...
                                     SeismicParameters &seismic_parameters) 
{
  ...
  //#endif
  std::vector<double> seis_sample(nt);
  std::vector<std::vector<double> > seis_sample_v(25);
  std::vector<double> seis_sample_t(nt);
  for (size_t i = 0; i < 25; ++i){
    seis_sample_v[i] = seis_sample_t;
  ...
  for (size_t i = 0; i < nx; i++) {
    for (size_t j = 0; j < ny; j++) {
      ...
      topt = toptime.GetZ(x, y);
      //if (i == 285 && j == 412) {
      //  std::cout << "i, j, x, y, z, topt = " << i << " " << j << " " << x << " " << y << " " << z << " " << topt << "\n";
      //  std::cout << toptime.IsMissing(topt) << "\n";
      //}
      if (time_output == true) {
        if (toptime.IsMissing(topt) == false) {
          double t = t0 + 0.5 * dt;
          ...
          for (size_t k = 0; k < nt; k++) {
            ...
            //if (i == 285 && j > 881 && j < 907) {
              //fout << std::endl;
            //}
            for (size_t l = 0; l < rgridvec.size(); l++) {
              timegridvec[l](i, j, k) = static_cast<float>(seis[l]);
              if (i == 285 && j >= 882 && j < 907) {
                seis_sample_v[j-882][k] = seis[l];
                seis_sample_t[k] = t;
              }
            }
            t = t + dt;
          ...
          if (i == 285 && j == 412) {
            seismic_parameters.seismicOutput()->printVector(seis_sample, "seis_sample.txt");  
            seismic_parameters.seismicOutput()->printVector(seis_sample_t, "seis_sample_t.txt");  
          }
        } 
        else {
          for (size_t k = 0; k < nt; k++){
