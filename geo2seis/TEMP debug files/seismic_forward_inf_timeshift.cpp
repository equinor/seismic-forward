...
void SeismicForward::seismicForward(SeismicParameters &seismic_parameters) {
  ...
  if (seismic_parameters.modelSettings()->GetNMOCorr()){
    ...
    if (time_output || depth_output || timeshift_output) { //OK?
      ...
      std::vector<std::vector<double> > seis_sample_time(400);
      std::vector<std::vector<double> > seis_sample_timeshift(400);
      std::vector<std::vector<double> > seis_sample_depth(400);
      std::vector<double> seis_sample_t(twt_0.size());
      std::vector<double> seis_sample_z(z_0.size());
      for (size_t i = 0; i < 400; ++i){
        seis_sample_time[i] = seis_sample_t;
        seis_sample_timeshift[i] = seis_sample_t;
        seis_sample_depth[i] = seis_sample_z;
      }
      ...
      for (int il = 1350; il < 1351; il += il_step) {
        ...
        ++il_steps;
        xl_steps = 0;
        for (int xl = 1000; xl < 1400; xl += xl_step) {
        //for (int xl = xl_min; xl <= xl_max; xl +=xl_step) {
          ...
          if (generateTraceOk(seismic_parameters, i, j)) {
            ...
            if (nmo_depth_segy_ok || nmo_depth_stack_segy_ok || depth_storm_output) {
              ...
              if (nmo_depth_segy_ok) {
                convertSeis(twt_vec_extrapol, twt_0, zgrid_vec_extrapol, z_0, nmo_timegrid_pos, nmo_depthgrid_pos, seismic_parameters);
                seismic_parameters.seismicOutput()->writeSegyGather(nmo_depthgrid_pos, nmo_depth_segy, z_0, offset_vec, false, x,y);
                for (size_t k = 0; k < z_0.size(); ++k) {
                  seis_sample_depth[xl_steps - 1][k] = nmo_depthgrid_pos[k][0];
                }
              }
              if (nmo_depth_stack_segy_ok || depth_storm_output){
                convertSeis(twt_vec_extrapol, twt_0, zgrid_vec_extrapol, z_0, nmo_timegrid_stack_pos, nmo_depthgrid_stack_pos, seismic_parameters);
              }
            ...
            if (nmo_timeshift_segy_ok || nmo_timeshift_stack_segy_ok || timeshift_storm_output) {
              ...
              if (nmo_timeshift_segy_ok) {
                convertSeis(twt_vec_shift, twt_0, timeshiftgrid_vec, twt_0, nmo_timegrid_pos, nmo_timeshiftgrid_pos, seismic_parameters);
                seismic_parameters.seismicOutput()->writeSegyGather(nmo_timeshiftgrid_pos, nmo_timeshift_segy, twt_0, offset_vec, true, x,y);
                for (size_t k = 0; k < twt_0.size(); ++k) {
                  seis_sample_timeshift[xl_steps - 1][k] = nmo_timeshiftgrid_pos[k][0];
                  seis_sample_time[xl_steps - 1][k] = nmo_timegrid_pos[k][0];
                }
              }
              if (nmo_timeshift_stack_segy_ok || timeshift_storm_output){
                convertSeis(twt_vec_shift, twt_0, timeshiftgrid_vec, twt_0, nmo_timegrid_stack_pos, nmo_timeshiftgrid_stack_pos, seismic_parameters);
              }
          ...
          else { //zero trace outside eclipse grid
            //for (size_t k = 0; k < twt_0.size(); ++k) {
            //  nmo_timegrid_stack_pos[k][0] = 0.0;
            //  for (size_t off = 0; off < offset_vec.size(); ++off) {
            ...
            //}
            if (nmo_time_segy_ok){
              seismic_parameters.seismicOutput()->writeZeroSegyGather(nmo_time_segy, offset_vec, x,y);
            }
            if (prenmo_time_segy_ok){
              seismic_parameters.seismicOutput()->writeZeroSegyGather(prenmo_time_segy, offset_vec, x,y);
            }
            if (nmo_time_stack_segy_ok) {
              seismic_parameters.seismicOutput()->writeZeroSegyGather(nmo_time_stack_segy, zero_vec, x,y);
            }
            if (nmo_depth_segy_ok) {
              seismic_parameters.seismicOutput()->writeZeroSegyGather(nmo_depth_segy, offset_vec, x,y);
            }
            if (nmo_depth_stack_segy_ok) {
              seismic_parameters.seismicOutput()->writeZeroSegyGather(nmo_depth_stack_segy, zero_vec, x,y);
            }
            if (nmo_timeshift_segy_ok) {
              seismic_parameters.seismicOutput()->writeZeroSegyGather(nmo_timeshift_segy, offset_vec, x,y);
            }
            if (nmo_timeshift_stack_segy_ok){
              seismic_parameters.seismicOutput()->writeZeroSegyGather(nmo_timeshift_stack_segy, zero_vec, x,y);
            }
            if (twtx_segy_ok) {
              seismic_parameters.seismicOutput()->writeZeroSegyGather(twtx_segy, offset_vec, x,y);
            }
            if (time_storm_output) { //er dette nødvendig?
              for (size_t k = 0; k < nt; ++k){
                timegrid(i, j, k) = 0.0;
                //printstorm grid when finish loop
              }
      ...
      seismic_parameters.seismicOutput()->printMatrix(seis_sample_timeshift, "seis_sample_timeshift_NMO.txt");  
      seismic_parameters.seismicOutput()->printMatrix(seis_sample_time, "seis_sample_time_NMO.txt");  
      seismic_parameters.seismicOutput()->printMatrix(seis_sample_depth, "seis_sample_depth_NMO.txt");  
                                 ...
                                 SeismicParameters &seismic_parameters)
{
  ...
  seismic_parameters.seismicOutput()->printVector(zt_reg, "zt_reg.txt");
  std::vector<double> t_test = NRLib::Interpolation::Interpolate1D(zt_reg, twt_0, z_0, "linear");
  seismic_parameters.seismicOutput()->printVector(t_test, "t_test.txt");
                                     ...
                                     SeismicParameters &seismic_parameters) 
{
  ...
  std::vector<std::vector<double> > seis_sample_timeshift(400);
  std::vector<std::vector<double> > seis_sample_depth(400);
  std::vector<double> seis_sample_t(nt);
  std::vector<double> seis_sample_z(nz);
  for (size_t i = 0; i < 400; ++i){
    seis_sample_time[i] = seis_sample_t;
    seis_sample_timeshift[i] = seis_sample_t;
    seis_sample_depth[i] = seis_sample_z;
  ...
  std::ofstream fout;
  NRLib::OpenWrite(fout, "seismic_cample_grid.txt");
        ...
        if (toptime.IsMissing(topt) == false) {
          ...
          for (size_t k = 0; k < nt; k++) {
            ...
            for (size_t l = 0; l < rgridvec.size(); l++) {
              ...
              if (i == 285 && j >= 132 && j < 532) {
                seis_sample_time[j-132][k] = seis[l];
              }
            }
            t = t + dt;
          }
        ...
        if (toptime.IsMissing(topt) == false) {
          ...
          for (size_t k = 0; k < nz; k++) {
            ...
            for (size_t l = 0; l < rgridvec.size(); l++) {
              ...
              if (i == 285 && j >= 132 && j < 532) {
                seis_sample_depth[j-132][k] = seis[l];
              }
            }
            z = z + dz;
          ...
          //if (i = 285 && j == 1307){
          //  seismic_parameters.seismicOutput()->printVector(t_vec, "t_vec_DC.txt");  
          //  seismic_parameters.seismicOutput()->printVector(twt, "twt_vec_DC.txt");  
          //  seismic_parameters.seismicOutput()->printVector(zvec, "z_vec_DC.txt");  
          //}
        } else {
          for (size_t k = 0; k < nz; k++) {
            for (size_t l = 0; l < rgridvec.size(); l++) {
        ...
        if (toptime.IsMissing(topt) == false) {
          ...
          for (size_t k = 0; k < nt; k++) {
            ...
            for (size_t l = 0; l < rgridvec.size(); l++) {
              ...
              if (i == 285 && j >= 132 && j < 532) {
                seis_sample_timeshift[j-132][k] = seis[l];
              }
            }
            tt = tt + dt;
  ...
  seismic_parameters.seismicOutput()->printMatrix(seis_sample_timeshift, "seis_sample_timeshift_OLD.txt");  
  seismic_parameters.seismicOutput()->printMatrix(seis_sample_time, "seis_sample_time_OLD.txt");  
  seismic_parameters.seismicOutput()->printMatrix(seis_sample_depth, "seis_sample_depth_OLD.txt");  
  // stop=omp_get_wtime();
