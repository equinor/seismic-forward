                                      ...
                                      std::vector<size_t> &n_max)
{
  ...
  for (size_t i = 0; i < offset_vec.size(); ++i) {
    ...
    std::cout << "twtx_min, max = " << twtx_min << " " << twtx_max << "\n";
    if (twtx_min > tmin){
                                    ...
                                    bool                      include_regular)
{
  ...
  //generate vrms_vec - only for each layer in reservoir
    ...
    int test = 1;
    //if ((*twtgrid_)(i, j, 0) == -999.0) {
    //  for (size_t k = 0; k < (*twtssgrid_).GetNK(); ++k) {
    //    vrms_vec(k, 1) = -999.0;
    //    vrms_vec(k, 0) = -999.0;
    //  }
    //}
    //else {
    //  v_over = 2000 * ((*zgrid_)(i, j, 0) - z_w) / ((*twtssgrid_)(i, j, 0) - 2000 * z_w / v_w);
    //  tmp0 = v_w*v_w*twt_w + v_over*v_over*((*twtssgrid_)(i, j, 0) - twt_w);
    //  for (size_t k = 0; k < nk; ++k) {
    //    tmp = tmp0;
    //    for (size_t l = 1; l <= k; ++l) {
    //      tmp += (*vsgrid_)(i, j, l)* (*vsgrid_)(i, j, l)*((*twtssgrid_)(i, j, l) - (*twtssgrid_)(i, j, l - 1));
    //    }
    //    vrms_vec(k, 1) = std::sqrt(tmp / (*twtssgrid_)(i, j, k));
    //  }
    //  v_over = 2000 * ((*zgrid_)(i, j, 0) - z_w) / ((*twtppgrid_)(i, j, 0) - 2000 * z_w / v_w);
    //  tmp0 = v_w*v_w*twt_w + v_over*v_over*((*twtppgrid_)(i, j, 0) - twt_w);
    //  for (size_t k = 0; k < nk; ++k) {
    //    tmp = tmp0;
    //    for (size_t l = 1; l <= k; ++l) {
    //      tmp += (*vpgrid_)(i, j, l)* (*vpgrid_)(i, j, l)*((*twtppgrid_)(i, j, l) - (*twtppgrid_)(i, j, l - 1));
    //    }
    //    vrms_vec(k, 0) = std::sqrt(tmp / (*twtppgrid_)(i, j, k));
    //  }
    //}
    ...
    //  std::vector<double> constvp = model_settings_->GetConstVp();
    //  double twt_wavelet = 2000 / constvp[2] * wavelet_->GetDepthAdjustmentFactor();
    //  double vrms_pp_under = vrms_vec(nk - 1, 0)*vrms_vec(nk - 1, 0)*(*twtppgrid_)(i, j, (nk - 1))   + constvp[2] * constvp[2] * twt_wavelet;
    //  double vrms_ss_under = vrms_vec(nk - 1, 1)*vrms_vec(nk - 1, 1)*(*twtssgrid_)(i, j, (nk - 1)) + constvs[2] * constvs[2] * twt_wavelet;
    //  vrms_pp_under *= (1 / ((*twtppgrid_)(i, j, (nk - 1))   + twt_wavelet));
    //  vrms_ss_under *= (1 / ((*twtssgrid_)(i, j, (nk - 1)) + twt_wavelet));
    //  vrms_pp_under = std::sqrt(vrms_pp_under);
    //  vrms_ss_under = std::sqrt(vrms_ss_under);

    //  //sample vrms regularly:
    //  std::vector<double> twt_vec_in(nk + 2), twt_vec_ss_in(nk + 2), twt_vec_pp_in(nk + 2), vrms_vec_pp_in(nk + 2), vrms_vec_ss_in(nk + 2);
    //  twt_vec_in[0] = twt_w;
    //  vrms_vec_pp_in[0] = v_w;
    //  vrms_vec_ss_in[0] = v_w;
    //  twt_vec_in[1] = 0.5*((*twtssgrid_)(i, j, 0) + (*twtppgrid_)(i,j,0));
    //  vrms_vec_pp_in[1] = vrms_vec(0, 0);
    //  vrms_vec_ss_in[1] = vrms_vec(0, 1);
    //  size_t index = 2;
    //  for (size_t k = 0; k < nk; ++k) {
    //    if (0.5*((*twtssgrid_)(i, j, k)+ (*twtppgrid_)(i, j, k)) != twt_vec_in[index - 1]) {
    //      twt_vec_in[index]     = 0.5*((*twtssgrid_)(i, j, k) + (*twtppgrid_)(i, j, k));
    //      vrms_vec_pp_in[index] = vrms_vec(k, 0);
    //      vrms_vec_ss_in[index] = vrms_vec(k, 1);
    //      ++index;
    //    }
    //  }
    //  twt_vec_in.resize(index + 1);
    //  vrms_vec_pp_in.resize(index + 1);
    //  vrms_vec_ss_in.resize(index + 1);
    //  twt_vec_in[index]     = twt_vec_in[index - 1] + twt_wavelet;
    //  vrms_vec_pp_in[index] = vrms_pp_under;
    //  vrms_vec_ss_in[index] = vrms_ss_under;
    //  //std::cout << "extrapol, twt, vrms_pp, vrms_ps " << twt_vec_in[index - 1] + twt_wavelet << " " << vrms_pp_under << " " << vrms_ss_under << "\n";
    //  //seismic_output_->printVector(twt_vec_in, "twt_vec_in.txt");
    //  //seismic_output_->printVector(vrms_vec_in, "vrms_vec_in.txt");

    //  std::vector<double> vrms_vec_reg_tmp_pp = NRLib::Interpolation::Interpolate1D(twt_vec_in, vrms_vec_pp_in, twt_0, "linear");
    //  std::vector<double> vrms_vec_reg_tmp_ss = NRLib::Interpolation::Interpolate1D(twt_vec_in, vrms_vec_ss_in, twt_0, "linear");
    //  for (size_t i = 0; i < vrms_vec_reg_tmp_pp.size(); ++i) {
    //    vrms_vec_reg(i, 0) = vrms_vec_reg_tmp_pp[i];
    //    vrms_vec_reg(i, 1) = vrms_vec_reg_tmp_ss[i];
    //  }
    //}
  }
}
                                        ...
                                        bool ps_seis)
{
  ...
  if (true) {
    ...
    for (size_t i = 0; i < (*twtgrid_).GetNI(); ++i) {
      for (size_t j = 0; j < (*twtgrid_).GetNJ(); ++j) {
        if (0.5*((*twtgrid_)(i, j, k_max)+(*twtssgrid_)(i,j,k_max)) > max_value) {
          max_value = 0.5*((*twtgrid_)(i, j, k_max) + (*twtssgrid_)(i, j, k_max));
          i_max = i;
          j_max = j;
        }
      }
                                          ...
                                          bool                 ps_seis)
{
  if (model_settings_->GetNMOCorr()){
    ...
    if (model_settings_->GetTwtFileName() != "") {
      twts_0 = GenerateTWT0Shift(twt_0[0], time_samples_stretch, ps_seis);
    }
  }
  else {
    ...
    if (model_settings_->GetTwtFileName() != "") {
      twts_0 = GenerateTWT0Shift(twt_0[0], twt_0.size(), ps_seis);
    }
  }
}
...
std::vector<double> SeismicParameters::GenerateTwt0ForNMO(size_t & time_stretch_samples,
  ...
  FindMaxTwtIndex(i_max, j_max, max_twt_value, ps_seis);
  ...
  if (!ps_seis) {
    ...
    std::vector<double> vrms_vec_0(nzrefl);
    for (size_t k = 0; k < nzrefl; ++k) {
      twt_vec[k] = (*twtgrid_)(i_max, j_max, k);
      vp_vec[k]  = (*vpgrid_) (i_max, j_max, k);
    }
    FindVrms(vrms_vec_0, dummy, twt_vec, dummy, vp_vec, 1.0, i_max, j_max, false);

    findVrmsPos(vrms_vec, dummy, dummy, i_max, j_max, false);
    seismic_output_->printVector(vrms_vec, "vrms_vec.txt");
    seismic_output_->printVector(vrms_vec_0, "vrms_vec_0.txt");
  ...
  twtx_max += 2 * twt_wavelet;

  //find tmin
  double factor = 2 * twtx_max / seismic_geometry_->tmax();
  double tmin = t0;
  size_t xtra_samples_top = 0;
  if (factor > 3) { //dette kriteriet er kanskje for svakt etter at 2 * wave er lagt til twtx_max
                                                ...
                                                const std::vector<double> &offset)
{
  ...
  size_t n_it = 10;
  size_t n_it_avg = 0;
  double theta_down, y_out, dU, dD, vr;
  ...
  for (size_t off = 0; off < offset.size(); off++) {
    ...
    for (size_t k = 0; k < twt_pp_vec.size(); k++) {
      //double tmp = offset[off] / (vrms_pp_vec[k]*twt_pp_vec[k] / 1000);
      //start_value = atan(tmp);
      ...
      n_it_avg += n_it;
      //std::cout << "it = " << n_it << "\n";
      thetagrid(k, off) = asin(vr*y_out);
      theta_down = asin(y_out);
      offset_down_grid(k, off) = tan(theta_down)*dD;
      offset_up_grid(k, off) = tan(thetagrid(k, off))*dU;
      start_value = y_out;
    }
  }
                                                          ...
                                                          size_t &n_it)
{
  ...
  for (size_t i = 0; i < n_it; ++i) {
    ...
    //if (f_der_y == 0) {
    //  std::cout << "failure in newtons method";
    //  return 0;
    //}
    y_new = y_old - f_y / f_der_y;

    if (std::abs(y_new) > 1.0)
      std::cout << "y storre enn 1. y_old = " << y_old << " y_new = " << y_new << "\n";
    //Fixed point iteration
    //y_new = offset * sqrt(1.0 - std::pow(y_old, 2)) / dD - dU * vr * y_old * sqrt(1 - std::pow(y_old, 2)) / (dD*sqrt(1 - std::pow(vr, 2)*std::pow(y_old, 2)));
    //y_new = sqrt(1 - std::pow(y_old, 2)) / dD *(offset - dU*vr*y_old / sqrt(1 - std::pow(vr, 2)*std::pow(y_old, 2)));
    //f_y = -1*offset +dD*y_new / sqrt(1.0 - std::pow(y_new, 2)) + dU*vr*y_new / sqrt(1.0 - std::pow(vr, 2) * std::pow(y_new, 2));
