...
void SeismicForward::MakeSeismic(SeismicParameters &seismic_parameters) 
{
  if (seismic_parameters.GetTimeOutput() || seismic_parameters.GetDepthOutput() || seismic_parameters.GetTimeshiftOutput()) {
    ...
    //std::cout << n_threads << " threads used.\n";
                                              ...
                                              GenSeisTraceParams *param)
{
  ...
  while (param->seismic_traces.try_pop(trace)){
    ...
    if (GenerateTraceOk(param->seismic_parameters, i, j)) {
      ...
      NRLib::Grid2D<double> &theta_pos                   = result_trace->GetThetaRefl();
      NRLib::Grid2D<double> &refl_pos                    = result_trace->GetRefl();
      NRLib::Grid2D<double> &twtx                        = result_trace->GetTWTxRefl();
      ...
      double tmax                 = param->seismic_parameters.seismicGeometry()->tmax() + 2 * twt_wavelet;
      ...
      //param->seismic_parameters.seismicOutput()->printVector(vrms_pp_vec, "vrms_pp_vec.txt");
      //param->seismic_parameters.seismicOutput()->printVector(vrms_pp_vec_reg, "vrms_pp_vec_reg.txt");
      //param->seismic_parameters.seismicOutput()->printVector(vrms_ss_vec, "vrms_ss_vec.txt");
      ...
      if (ps_seis) {
        ...
        FindPSTWTx(twtx, twt_ss_vec, twt_pp_vec, vrms_pp_vec, vrms_ss_vec, offset_ss, offset_pp);

        //sample twtx regularly - for each layer for each offset:
        ResampleForTwtxReg(twt_vec, twt_pp_vec, twt_ss_vec, offset_pp, offset_ss, param->twt_0, twt_pp_vec_reg, twt_ss_vec_reg, offset_pp_reg, offset_ss_reg);
        FindPSTWTx(twtx_reg, twt_ss_vec_reg, twt_pp_vec_reg, vrms_pp_vec_reg, vrms_ss_vec_reg, offset_ss_reg, offset_pp_reg);

        //find limits for where to generate seismic, for each offset
        FindSeisLimits(twtx_reg, param->twt_0, n_min, n_max, tmax);
        ...
        //param->seismic_parameters.seismicOutput()->printVectorSizeT(n_min, "n_min3.txt");
        //param->seismic_parameters.seismicOutput()->printVectorSizeT(n_max, "n_max3.txt");
      }      
      ...
      //param->seismic_parameters.seismicOutput()->printMatrix(refl_pos, "reflections.txt");
      //param->seismic_parameters.seismicOutput()->printMatrix(theta_pos, "theta_pos.txt");
      ...
      //ResampleTwtx(twtx, twtx_reg, twt_vec, param->twt_0);
                                ...
                                size_t                      &max_sample)
{
  ...
  for (size_t off = 0; off < noff; off++) {
    ...
    if (index > data_out.GetNI()){
      std::cout << "ERROR: stretch not properly accounted for, inside = " << inside << "\n";
      std::cout << "index = " << index << " ni = " << data_out.GetNI() << "\n";
    }
    //for (size_t k = 0; k < index; k++) {
    if (max_sample > data_out.GetNI())
      max_sample = data_in.GetNI();
    for (size_t k = 0; k < index; k++) {
...
void SeismicForward::ResampleForTwtxReg(const std::vector<double>   &twt_vec, 
                                        ...
                                        const std::vector<double>   &twt_0,
                                        std::vector<double>         &twt_pp_vec_reg,
                                        std::vector<double>         &twt_ss_vec_reg,
                                        NRLib::Grid2D<double>       &offset_pp_reg,
                                        NRLib::Grid2D<double>       &offset_ss_reg)
{

  //twt_pp_vec_reg = LinInterp1D(twt_vec, twt_pp_vec, twt_0);
  //twt_ss_vec_reg = LinInterp1D(twt_vec, twt_ss_vec, twt_0);
...
void SeismicForward::FindPSTWTx(NRLib::Grid2D<double>     &twtx_grid,
                                const std::vector<double> &twt_ss_vec,
                                const std::vector<double> &twt_pp_vec,
                                ...
                                const NRLib::Grid2D<double>  &offset_pp)
{
  ...
  for (size_t off = 0; off < offset_ss.GetNJ(); ++off) {
    for (size_t k = 0; k < twt_pp_vec.size(); ++k) {
      twtx_pp = std::sqrt(twt_pp_vec[k] * twt_pp_vec[k] / 4 + 2000 * 2000 * (offset_pp(k,off) * offset_pp(k,off) / (vrms_pp_vec[k] * vrms_pp_vec[k])));
      twtx_ss = std::sqrt(twt_ss_vec[k] * twt_ss_vec[k] / 4 + 2000 * 2000 * (offset_ss(k,off) * offset_ss(k,off) / (vrms_ss_vec[k] * vrms_ss_vec[k])));
                                       ...
                                       double                       tmax)
{
  ...
  for (size_t off = 0; off < n_min.size(); ++off) {
    std::cout << "NEW: twtx_min, max, tmax = " << twtx_grid(0, off) << " " << twtx_grid(index, off) << " " << tmax <<  "\n";
  ...
  if (toptime.IsMissing(topt) == false) {
    for (size_t off = 0; off < offset.size(); off++) {
      ...
      for (size_t k = 0; k < nt; k++) {
        if (k > (n_min[off]-1) && k < (n_max[off]+1)) {
          ++count_in;
          seis = 0.0;
